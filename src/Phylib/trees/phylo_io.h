...
namespace Phylib {
	...
	template<typename T> void read_newick(istream& is, phylo<T>& tree, vector<string>& taxa_names, double defaultLength, bool toplevel, bool checkTaxa) { //reads in tree in phylip format.
		...
		if (next_tok == io_tree_left) {
			...
			while (true) { //Loop through subtrees
				phylo<T> subtree;
				read_newick<T>(is, subtree, taxa_names, defaultLength, checkTaxa);
				if (new_child.null()) {
				...
				if (next_tok==io_tree_right) {
					if (new_node.left().null() )
						throw  PhylibException("Error: Error in Newick syntax");
					else
						break;
				} else if (next_tok!=io_tree_comma) {
					throw  PhylibException("Error: Expected ',' or ')' ");
				}
			}
			next_tok = peek_next_tree_token<T>(is);
		}
...
//            if (next_word.compare(0, 10, taxa_names[id], 0, 10) == 0) {
                if (next_word==taxa_names[id]) {
                    //cerr<<"Found "<<next_word<<" which equals taxon number "<<id<<endl;
                    break;
			...
			if (id==taxa_names.size()) {
                if (checkTaxa)
                    throw PhylibException("Taxon "+next_word+" in tree is not present in sequence data");
                else {
			...
			if (at_leaf) {
                ...
                cerr<<"Internal Label"<<endl;
            }
			next_tok = peek_next_tree_token<T>(is);
		} else {
			if (at_leaf) {
				throw  PhylibException("Error: Expected taxon name");
			}
		}
		//Check for metadata
		...
		if (next_tok == io_tree_semicolon) {
			next_tree_token<T>(is, next_word);
			if (!toplevel)
				throw  PhylibException("Error: Read ';' in middle of tree description");
			return;
		}
	...
	template<typename T> void read_newick(istream& is, phylo<T>& tree, vector<string>& taxa_names, double default_branchlength = 0.0, bool checkTaxa = true) {
		using namespace std;
		read_newick<T>(is, tree, taxa_names, default_branchlength, true, checkTaxa);
		return;
	}
