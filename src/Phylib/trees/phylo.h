...
//TODO [22/10/18]: Change read_newick so that it can read a phylo of a generic type.
...
public:
	...
	public:
            ...
            /**
             * Right child.
             * @return iterator pointing to second child, or null iterator if there is none
             */
            inline iterator rchild() const {
                if (left().null())
                    return left();
                else
                    return left().right();
            ...
            /**
                        If this is the only child then returns null. Otherwise returns next sibling to right, or first sibling if this is the rightmost.
             */
            inline iterator nextsib() const {
                if (!right().null())
                    return right();
			...
			iterator next_pre(iterator subtreeRoot=iterator(0)) const {
				if (this->header()) {
				...
				iterator p = *this;
				while (!p.header() && p!=subtreeRoot && p.right().null()) {
					p=p.par();
				}
				if (p.header() || p==subtreeRoot) {
					return iterator(0);
				} else {
					return p.right();
				}
			...
			iterator next_post(iterator subtreeRoot=iterator(0)) {
	...
	public:
            ...
            /**
             * Right child.
             * @return const_iterator pointing to second child, or null const_iterator if there is none
             */
            inline const_iterator rchild() const {
                if (left().null())
                    return left();
                else
                    return left().right();
        ...
        void erase(iterator position) {
            ...
            while (!position.left().null()) {
                erase(position.left()); // erase children first
            }
            if (!position.header()) {

                if (position == position.par().left()) {
                    (position.par().current)->left = (position.current)->right; // deleting leftmost child
                } else {
                    iterator leftsib = position.par().left(); // Find sibling to the left of position.
                    while (!leftsib.null() && leftsib.right() != position) {
                        leftsib = leftsib.right();
                    }
                    if (leftsib.null()) {
                        throw Phylib::PhylibException(
                            "erase: position not found among parent's children"
                        );
                    }
        ...
        /**
            Takes two children of the same node and inserts a new node, making the two siblings the  children
            of that node.
         @param sib1 iterator pointing to a node in the tree
         @param sib2 a different iterator, pointing to a sibling of sib1
         @param x phylogeny
         @return iterator pointing to the new parent node
         */
        iterator merge_siblings(iterator sib1, iterator sib2, phylo<T>& x) {
            if (sib1.par()!=sib2.par())
                throw PhylibException("Trying to merge nodes which are not siblings");
            if (sib1==sib2)
                throw PhylibException("Trying to merge a node with itself");
            iterator newParent = insert_sibling(sib1);
            graft_child(newParent,x,sib1);
            graft_sibling(sib1,newParent,sib2);
            return newParent;
        }
	...
	protected:
        ...
        void detach(iterator position) {
            if (position == position.par().left()) {
                (position.par().current)->left = (position.current)->right;
            } else {
                iterator leftsib = position.par().left();
                while (!leftsib.null() && leftsib.right() != position) {
                    leftsib = leftsib.right();
                }
                if (leftsib.null()) {
                    throw PhylibException(
                        "detach: position not found among parent's children"
                    );
                }
                (leftsib.current)->right = (position.current)->right;
            }
...
template <typename T> bool check_topology(const phylo<T>& tree) {
    for (auto p = tree.root();!p.null();p=p.next_pre()) {
        //Check all children have the correct parent.
        for (auto q = p.left();!q.null();q=q.right()) {
            if (q.par()!=p) {
                cerr<<"Tree linkage problem"<<endl;
                return false;
            }
        }
    }
